...
pub fn sm_mul64(mut a: u64, mut b: u64, m: u64) -> u64 {
    let mut result = 0;
    while b > 0 {
        if b & 1 != 0 {
            result = (result + a) % m;
...
pub fn sm_mul(mut a: u8, mut b: u8, m: u8) -> u8 {
    let mut result = 0;
    while b > 0 {
        if b & 1 != 0 {
            result = (result + a) % m;
...
pub fn big_mul64(mut a: u64, mut b: u64, m: u64) -> u64 {
...
pub fn big_mul(mut a: u8, mut b: u8, m: u8) -> u8 {
    // precompute 2^8 mod m, since we expect to wrap
    let Wrapping(msb_mod_m) = Wrapping(MAX_U8) - Wrapping(m) + Wrapping(1);
    let msb_mod_m = msb_mod_m % m;
    //assert!(msb_mod_m > 128);
